  ...
  properties:
    ...
    keyType:
      ...
      enum:
        ...
        - RSA_PUB_EXT
        - DH_PUB
        # DH_PAIR is not yet supported: SPSDK has no DH key implementation to derive
        # the public key from a private key. Will be enabled once DH support is added.
        # - DH_PAIR
      template_value: AES
        ...
        - type: object
          ...
          properties:
            pubExponentSize:
              ...
              description: Size of RSA public exponent in bytes. Maximum is device-dependent (8 for HSE-B, 16 for HSE-H/M).
              format: number
              maximum: 16
              template_value: 3
...
smr:
  ...
  properties:
    ...
    configFlags:
      ...
      description: |
        A combination of configuration flags between a memory interface
        and the authenticity proof used for verification, as specified in
        Additional SMR configuration flags.
      items:
        type: string
        enum: [QSPI_FLASH, SD_FLASH, MMC_FLASH, INSTALL_AUTH, AUTH_AAD]
        template_value: []
      uniqueItems: true
      enum_template: [QSPI_FLASH, SD_FLASH, MMC_FLASH, INSTALL_AUTH, AUTH_AAD]
    ...
    authKeyHandle:
      ...
      description: |
        ...
        Use the catalog form (catalogId/groupIdx/slotIdx) for keys stored in the HSE key catalog.
        Use the address form (keyAddress) for external public keys (HSE_KEY_TYPE_*_PUB_EXT)
        stored in application memory.
      oneOf:
        - type: object
          title: Catalog Key Handle
          description: Key handle referencing a key in the HSE key catalog (ROM, NVM, or RAM)
        ...
        - type: object
          title: Address Key Handle
          description: |
            Key handle as a direct memory address for HSE_KEY_TYPE_*_PUB_EXT keys
            stored in application memory (e.g., in flash).
          ...
          properties:
            keyAddress:
              ...
              title: Key Address
              description: |
                Address of the public key in application memory.
                Used for HSE_KEY_TYPE_ECC_PUB_EXT and HSE_KEY_TYPE_RSA_PUB_EXT key types.
              format: number
              minimum: 0
              maximum: 0xFFFFFFFF
              template_value: "0x0"
    ...
    instAuthTagAddrs:
      ...
      minItems: 2
      maxItems: 2
      skip_in_template: True # this will be calculated automatically when installing the SMR
      items:
        type: [number, string]
        format: number
        template_value: [0, 0]
...
cr:
  type: object
  title: HSE Core Reset Entry
  description: Configuration for a Core Reset entry that defines the parameters for advanced secure boot configurations for Application Cores
  required: [output, coreId, crSanction, startOption]
  properties:
    output:
      type: string
      title: Output file name
      ...
      format: file_name
      template_value: cr_entry.bin

    coreId:
      type: string
      title: Core ID
      description: Identifies the core ID to be started (application core mapping)
      enum: [M7_0, M7_1, M7_2, M7_3] # will be replaced with actual core IDs based on device
      template_value: M7_0

    crSanction:
      type: string
      title: Core Reset Sanction
      description: |
        The sanction applied if one of the SMR(s) linked to the CR entry failed verification.
      ...
      enum: [KEEP_CORE_IN_RESET, DIS_INDIV_KEYS, RESET_SOC]
      template_value: KEEP_CORE_IN_RESET

    preBootSmrMap:
      title: Pre-Boot SMR Map
      description: |
        The PRE-BOOT SMR(s) which need to be verified before releasing the core from passResetAddr address.
        Can be specified as:
        ...
        Keep in mind, the size of SMR entry table may be limited by device.
      oneOf:
        - type: [number, string]
          format: number
          minimum: 0
    ...
    passResetAddr:
      type: [number, string]
      title: Pass Reset Address
      description: |
        The primary address of the first instruction after a regular reset.
      ...
      format: number
      minimum: 0
      template_value: "0x00401000"

    altPreBootSmrMap:
      title: Alternative Pre-Boot SMR Map
      description: |
        The ALT-PRE-BOOT SMR(s) which need to be verified before releasing the core from altResetAddr address.
    ...
    altResetAddr:
      type: [number, string]
      title: Alternative Reset Address
      description: |
        The alternative address of the first instruction after a regular reset.
    ...
    postBootSmrMap:
      title: Post-Boot SMR Map
      description: |
        The POST-BOOT SMR(s) which need to be loaded after verifying the preBootSmrMap SMR(s).
        It's a 32-bit value, each bit specifying the particular SMR entry index from 0-31.
    ...
    startOption:
      ...
      title: Start Option
      description: |
        Specifies if the Application Core is automatically released from reset or requires manual intervention.
      ...
      enum: [AUTO_START, MANUAL_START]
      template_value: AUTO_START
